...
volatile sig_atomic_t global_pid = 0;
...
void eval(char *cmdline) {
  ...
  char *argv[MAXARGS];
  pid_t pid;
  sigset_t mask_all, pre_one, mask_one;
 ...
 */
// TODO:SIGINT function
void sigint_handler(int sig) {
  pid_t pid;
  if ((pid = fgpid(jobs)) > 0) { // only if foreground job exist
    kill(-fgpid(jobs), SIGINT);  // if
